...
for run in range(runs):
	for brainSize in M:
		for nOfAgents in N:
			# run games with von neumann neighbourhood
			for vonNeumannRadius in dVonNeumann:
				i+=1
				g, cgs = generateVonNeumannGameComunities(nOfAgents, brainSize, S, vonNeumannRadius)

				print "{} - Starting the Game {} of total: {}, number of cgames: {}".format(datetime.datetime.now(), i, totalGamesVonNeumann, len(cgs))
				runComunityRound(g, cgs, nRounds)
				
				nOfNeigh = len(cgs[0].agents)
			...
			# run a full game
			i+=1
			g, cgs = generateFixedGameComunities(nOfAgents, brainSize, S, nOfAgents)
